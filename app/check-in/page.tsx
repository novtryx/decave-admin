...
import { IoTicketOutline, IoWineOutline } from "react-icons/io5";
...
import CocktailRedemptionPanel from "@/components/checkin/CocktailRedemptionPanel";

type CheckInTab = "tickets" | "drinks";
...
export default function CheckInPage() {
  const [activeTab, setActiveTab] = useState<CheckInTab>("tickets");
  ...
  return (
    <DashboardLayout>
      ...
      {!eventId ? (
        ...
        <>
          {/* Tab switch — tickets vs drinks share the event picker
              above, but scan/redeem flows are independent below. */}
          <div className="flex gap-2 mb-6 border-b border-[#2a2a2a]">
            <button
              onClick={() => setActiveTab("tickets")}
              className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 -mb-px transition-colors ${
                activeTab === "tickets"
                  ? "border-[#cca33a] text-white"
                  : "border-transparent text-[#6F6F6F] hover:text-white"
              }`}
            >
              <IoTicketOutline /> Scan Tickets
            </button>
            <button
          ...
          {activeTab === "tickets" ? (
            ...
            // Drinks tab — cocktail codes are looked up by their own
            // txnId, independent of the event selected above (an
            // order only ever belongs to one event anyway).
            <CocktailRedemptionPanel />
          )}
        </>
      )}
    </DashboardLayout>
