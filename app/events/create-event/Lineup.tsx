...
export default function Lineup({ step, setStep }: StepProps) {
  ...
  const handleImageUploadComplete = (artistId: number, imageData: { url: string }) => {
    ...
    if (!imageData) {
      ...
      setSubmitError("Failed to upload image. No data received.");
      return;
    }

    if (!imageData.url) {
      console.error("❌ No URL in image data:", imageData);
      setSubmitError("Failed to upload image. No URL in response.");
      return;
    }

    if (typeof imageData.url !== 'string' || imageData.url.trim() === '') {
      console.error("❌ Invalid URL format:", imageData.url);
      setSubmitError("Failed to upload image. Invalid URL format.");
      return;
    }
    ...
    console.log("✅ Found artist at index:", artistIndex);
    console.log("Current artist data:", artists[artistIndex]);

    // Update the artist's image URL
    console.log("Updating artist imageUrl to:", imageData.url);
    updateArtist(artistId, 'imageUrl', imageData.url);
    ...
    console.log("✅ Image upload complete and artist updated");
  }
  ...
  const handleSaveHeadliner = async () => {
    ...
    try {
      ...
      console.log("Valid artists to submit:", validArtists);
  ...
  return (
    <div className="text-white">
      ...
      {/* Debug Info */}
      <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500 rounded text-xs space-y-1">
        <p className="text-blue-400">
          Artists in store: {artists.length}
        </p>
        <p className="text-blue-400">
        ...
        {artists.map((artist, idx) => (
          <p key={artist.id} className="text-blue-300">
            Artist {idx + 1}: Name={artist.name || 'empty'} | Role={artist.role || 'empty'} | Image={artist.imageUrl ? '✓' : '✗'}
          </p>
        ))}
      </div>
              ...
              <div>
                <ImageUpload
                  ...
                  onUploadComplete={(imageData) => {
                    console.log("ImageUpload onUploadComplete triggered");
                    console.log("Received imageData:", imageData);
                    handleImageUploadComplete(artist.id, imageData);
                  }}
                  helperText="JPG, JPEG, PNG (Max 1MB)"
                  previewClassName="h-40 w-48"
                  error={artistErrors.imageUrl}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={artist.name || ""}
                    onChange={(e) => {
                      console.log(`Updating artist ${artist.id} name to:`, e.target.value);
                      updateArtist(artist.id, 'name', e.target.value);
                      setErrors((prev) => {
                        const newArtists = { ...prev.artists };
                        if (newArtists?.[index]) {
                ...
                <div>
                  ...
                  <input
                    ...
                    value={artist.role || ""}
                    onChange={(e) => {
                      console.log(`Updating artist ${artist.id} role to:`, e.target.value);
                      updateArtist(artist.id, 'role', e.target.value);
                      setErrors((prev) => {
                        const newArtists = { ...prev.artists };
                        if (newArtists?.[index]) {
