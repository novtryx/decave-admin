...
import { CreateEventTicketAction, EditEventAction, UpdateEventTicketAction, CreateEventCocktailAction, UpdateEventCocktailAction } from "@/app/actions/event";
...
const handleSaveTicket = async () => {
  ...
  try {
    ...
    // ================================
    // Cocktails — same create/update split as tickets, only for
    // entries that actually have a name + price filled in.
    // ================================
    for (const cocktail of cocktails) {
      if (!cocktail.name.trim() || !cocktail.price) continue;

      if (cocktail._id) {
        const res = await UpdateEventCocktailAction(eventId, cocktail._id, {
        ...
        const res = await CreateEventCocktailAction({
          eventId,
          name: cocktail.name.trim(),
          description: cocktail.description.trim(),
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    {/* Suggestions only — this is a free-text label, not
                        an enum. "Tier Category" below is the canonical
                        categorization analytics relies on. */}
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm mb-2">
                            Price (₦) <span className="text-red-500">*</span>
                            {cocktail._id && (
                              <span className="text-gray-500 font-normal"> (locked after creation)</span>
                            )}
                          </label>
                          <input
                            type="number"
                            min="0"
                            disabled={!!cocktail._id}
                            value={cocktail.price}
                            onChange={(e) => updateCocktail(cocktail.id, "price", e.target.value)}
                            placeholder="0"
                            className="w-full bg-transparent border border-[#2a2a2a] rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-gray-600 disabled:opacity-50"
                          />
                        </div>
                        <div>
